...
const Header = () => {
    ...
    return (
        <header className="flex justify-between items-center p-4 text-black">
            ...
            <div
                className={`fixed inset-0 bg-black z-10 transition-opacity duration-700 ${showOverlay ? 'opacity-50' : 'opacity-0 pointer-events-none'}`}
                onClick={handleToggle}
            ></div>
    
            <div className={`fixed top-0 right-0 h-full w-1/2 text-black p-4 z-20 transition-transform duration-1000 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
